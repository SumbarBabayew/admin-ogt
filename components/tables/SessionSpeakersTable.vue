...
import { deleteSpeakerFromSession } from "@/service/admin.api";
const emit = defineEmits(["delete-item"]);
const props = defineProps({ items: Array, count: Number, chatroom: Number });
const handleItems = ref([])
const deleteLoading = ref(false)
const deleteItem = async (c) => {
  await deleteSpeakerFromSession({data: { speakerId: c, sessionId: Number(props.chatroom) }}).then(() => {
    ...
    <div class="container">
      ...
      <ul class="responsive-table">
        <li class="table-header">
          ...
          <div class="col col-1">Fullname</div>
        ...
        <li class="table-row" v-for="(item, i) in items">
          ...
          <div class="col col-1" data-label="Fullname">{{ item.nameEn }}</div>
          <div class="col col-2" data-label="Position">{{item.positionEn}}</div>
          
          <div class="col col-4" data-label="Functions">
            <v-btn size="30" class="profileBtn bg-primary custom-hover-primary" @click="$goPage(`/speakers/${item.id}`)"
