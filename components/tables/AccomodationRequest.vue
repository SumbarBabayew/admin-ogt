    ...
    <div class="container">
      ...
      <ul class="responsive-table">
        <li class="table-header">
          ...
          <div class="col col-2">Dates</div>
          <div class="col col-2">Event</div>
          <div class="col col-4">Functions</div>
        </li>
        <li class="table-row" v-for="(item, i) in items">
          <div class="col col-1" data-label="Firstname">{{ item.firstName }}</div>
          <div class="col col-2" data-label="Lastname">{{item.lastName}}</div>
          <div class="col col-2" data-label="Phone">{{item.phone}}</div>
          <div class="col col-2" data-label="Email">{{item.email}}</div>
          <div class="col col-2" data-label="Company">{{item.company}}</div>
          <div class="col col-2" data-label="Dates">{{new Date(item.checkInDate).toLocaleString()}} &<br> {{new Date(item.checkOutDate).toLocaleString()}}</div>
          <div class="col col-2" data-label="Event" style="color: green;">{{item.event != null ? item.event.name : ""}}</div>
          <div class="col col-4" data-label="Functions">
            <v-btn size="30" class="profileBtn bg-primary custom-hover-primary" @click="$goPage(`/accomodation-request/${item.id}`)"
