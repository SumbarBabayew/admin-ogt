    ...
    <div class="container">
      ...
      <ul class="responsive-table">
        ...
        <li class="table-row" v-for="(item, i) in items">
          <div class="col col-1" data-label="Title" :style="{ color: `#${item.color}` }">{{ item.session != null ? item.name : item.name }}</div>
          ...
          <div class="col col-4" data-label="Functions">
            <v-btn size="30" class="profileBtn bg-primary custom-hover-primary" @click="$goPage(`/events/${item.id}`)"
            ...
            <v-btn size="30" class="profileBtn bg-secondary custom-hover-primary" variant="text" @click="$goPage(`/events/hotels/${item.id}`)"
              v-bind="props">
              <BuildingSkyscraperIcon size="15" />
            </v-btn>
          </div>
        </li>
      </ul>
