    ...
    <div class="container">
      ...
      <ul class="responsive-table">
        <li class="table-header">
          ...
          <div class="col col-2">Functions</div>
        </li>
        <li class="table-row" v-for="(item, i) in items">
          <div class="col col-1" data-label="Start">{{ item.start.nameEn }}</div>
          <div class="col col-2" data-label="Destination">{{item.destination.nameEn}}</div>
          <div class="col col-2" data-label="Departure Time">{{item.departureTime}}</div>
          <div class="col col-2" data-label="Arrival Time">{{item.landingTime}}</div>
          <div class="col col-4" data-label="Functions">
            <v-btn size="30" class="profileBtn bg-primary custom-hover-primary" @click="$goPage(`/available-flights/${item.id}`)"
