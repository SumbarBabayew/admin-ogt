...
import { deleteCompanyFromCategory } from "@/service/admin.api";
const emit = defineEmits(["delete-item"]);
const props = defineProps({ items: Array, count: Number });
const handleItems = ref([])
const deleteLoading = ref(false)
const deleteItem = async (c) => {
  await deleteCompanyFromCategory({ data: {participantCategoryId: Number(route.params.id), participantCompanyId: c} }).then(() => {
