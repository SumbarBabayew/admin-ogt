    ...
    <div class="container">
      ...
      <ul class="responsive-table">
        <li class="table-header">
          <div class="col col-6">Title EN</div>
          <div class="col col-4">Event</div>
          <div class="col col-1">Functions</div>
        </li>
        <li class="table-row" v-for="(item, i) in items">
          <div class="col col-6" data-label="Title EN">{{ item.titleEn }}</div>
          <div class="col col-4" data-label="Event">{{ item.event != null ? item.event.name : "" }}</div>
          <div class="col col-1" data-label="Functions">
            <v-btn size="30" class="profileBtn bg-primary custom-hover-primary" @click="$goPage(`/about-conference/${item.id}`)"
