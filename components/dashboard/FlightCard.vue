...
const deleteItem = async (c) => {
  await deleteFlight({ id: c }).then(res => {
    ...
    <v-col cols="12" lg="3" sm="6" v-for="(card, i) in  props.items " :key="i">
      <v-card elevation="10" class="withbg store__card" rounded="md">
        <v-card-item class="pt-1 d-flex">
          <h6 class="text-h6">From: {{ card.start.nameEn }}</h6>
          <h6 class="text-h6">To: {{ card.destination.nameEn }}</h6>
          <h6 class="text-h6">DPT: {{ card.wqdepartureTime }}</h6>
          <h6 class="text-h6">LND: {{ card.landingTime }}</h6>
        </v-card-item>
        <div class="d-flex justify-end mr-4 pb-1   gap-2">
          <v-btn :to="`/available-flights/${card.id}`" size="40" icon class="bg-primary d-flex">
            <v-avatar size="30" class="text-white">
              <FilePencilIcon size="15" />
            </v-avatar><v-tooltip activator="parent" location="bottom">Edit </v-tooltip></v-btn>
