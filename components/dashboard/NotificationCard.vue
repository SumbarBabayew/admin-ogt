...
import { deleteNotification } from "@/service/admin.api";
...
const deleteItem = async (c) => {
  ...
  await deleteNotification({ id: c }).then(res => {
  ...
  <v-row>
    <v-col cols="12" lg="3" sm="6" v-for="(card, i) in  props.items " :key="i">
      <v-card elevation="10" class="withbg store__card" rounded="md">
        <NuxtLink :to="`/notifications/${card.id}`">
          <img crossorigin="anonymous" :src="`${baseUrl.url_base}/${card.preview}`" style="height:200px; width: 100%;"
            class="rounded-t-md" alt="123" />
        </NuxtLink>
        <div class="d-flex justify-end mr-4 mt-n5 gap-2">
          <v-btn :to="`/notifications/${card.id}`" size="40" icon class="bg-primary d-flex">
        ...
        <v-card-item class="pt-0 d-flex">
          <h6 class="text-h6">{{ card.titleEn }}</h6>
        </v-card-item>
      </v-card>
    </v-col>
  </v-row>
