...
import { deleteMadeCountry } from "@/service/admin.api";
const emit = defineEmits(["delete-item"]);
const props = defineProps({ items: Array });
const handleItems = ref([])
const deleteItem = async (c) => {
  await deleteMadeCountry({ id: c }).then(res => {
  ...
  <v-row>
    <v-col cols="12" lg="3" sm="6" v-for="(card, i) in  props.items " :key="i">
      <v-card elevation="10" class="withbg store__card" rounded="md">
        <NuxtLink :to="`/country/${card.id}`">
          <img crossorigin="anonymous" :src="`${baseUrl.url_base}${card.flag}`" style="height:200px; width: 100%;"
            class="rounded-t-md" alt="123" />
        </NuxtLink>
        <div class="d-flex justify-end mr-4 mt-n5 gap-2">
          <v-btn :to="`/country/${card.id}`" size="40" icon class="bg-primary d-flex">
            <v-avatar size="30" class="text-white">
              <FilePencilIcon size="15" />
            </v-avatar>
            <v-tooltip activator="parent" location="bottom">Duzet
            </v-tooltip>
          </v-btn>
          ...
          <v-btn size="40" icon class="bg-error d-flex" @click="deleteItem(card.id)">
            ...
            <v-tooltip activator="parent" location="bottom">Poz
            </v-tooltip>
          </v-btn>
        </div>
