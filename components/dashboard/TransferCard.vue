...
import { deleteTransfer } from "@/service/admin.api";
...
const deleteItem = async (c) => {
  ...
  await deleteTransfer({ id: c }).then(() => {
  ...
  <v-row>
    <v-col cols="12" lg="3" sm="6" v-for="(card, i) in  props.items " :key="i">
      <v-card elevation="10" class="withbg store__card" rounded="md">
        <NuxtLink :to="''">
          <img crossorigin="anonymous" :src="`${baseUrl.url_base}/${card.image}`" style="height:200px; width: 100%; object-fit:contain"
        ...
        <div class="d-flex justify-end  mt-n5">
          ...
          <v-menu :close-on-content-click="false">
            ...
            <v-sheet rounded="md" width="200" elevation="10" class="mt-2">
              <v-list class="py-0" lines="one" density="compact" @click="$goPage(`/transfers/features/${card.id}`, {})">
                <v-list-item value="item2" active-color="primary">
                  <template v-slot:prepend>
                    <DoorIcon size="20" />
                  </template>
                  <v-list-item-title class="pl-4 text-body-1">Features</v-list-item-title>
                </v-list-item>
              </v-list>
              <v-list class="py-0" lines="one" density="compact" @click="$goPage(`/transfers/${card.id}`, {})">
                <v-list-item value="item2" active-color="primary">
                  <template v-slot:prepend>
