...
import { deleteFeedback } from "@/service/admin.api";
...
const deleteItem = async (c) => {
  ...
  await deleteFeedback({ id: c }).then(() => {
    ...
    <v-col cols="6" lg="2" sm="3" v-for="(card, i) in  props.items " :key="i">
      <v-card elevation="10" class="withbg store__card" rounded="md">
        ...
        <div class="d-flex justify-end  mt-n5">
          ...
          <v-menu :close-on-content-click="false">
            ...
            <v-sheet rounded="md" width="200" elevation="10" class="mt-2">
              <v-list class="py-0" lines="one" density="compact" @click="$goPage(`/feedbacks/${card.id}`, {})">
        ...
        <v-card-item class="pt-0 d-flex">
          <h6 class="text-h6" v-text="`${card.fullnameEn}`"></h6>
