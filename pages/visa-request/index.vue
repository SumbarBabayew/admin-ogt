...
import { getVisaRequests, getEvents } from "@/service/admin.api";
import VisaRequest from '@/components/tables/VisaRequest.vue';
...
const localQuery =
  JSON.parse(localStorage.getItem('VisaRequestsQuery')) ||
  {
    page: 1,
    keyword: '',
    as: "",
...
watch(route, value => {
  localStorage.setItem('VisaRequestsQuery', JSON.stringify(query.value))
...
const searchEmpty = (s) => {
  if (s) {
    query.value.keyword = ""
    query.value.page = JSON.parse(localStorage.getItem('VisaRequestsQuery')).page || 1
    localStorage.setItem('VisaRequestsQuery', JSON.stringify(query.value))
    fetchItems()
  }
}
...
const fetchItems = async () => {
  const { data } = await getVisaRequests(JSON.parse(localStorage.getItem('VisaRequestsQuery')) || query.value)
...
<template>
  ...
  <div v-else>
    ...
    <VisaRequest :items="items" @delete-item="setDelete" :count="count" />
