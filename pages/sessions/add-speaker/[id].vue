<script setup>
import { ref } from "vue";
import MTextField from "../../../components/base/MTextField.vue";
import { addSpeakerToSession, getSessions, getSpeakers } from "@/service/admin.api";
import { useRoute, useRouter } from "vue-router";
import Loading from "~~/components/base/Loading.vue";
const router = useRouter();
const route = useRoute();
const error = ref(false);
const loading = ref(false);
const loadingAdd = ref(false);
const item = ref({
  speakerId: null,
  sessionId: parseInt(route.params.id),
  isModerator: false,
  isKeynote: false,
});
const speakers = ref([]);

const fetchData = async () => {
  const user = await getSpeakers({ limit: 300 });

  speakers.value = user.data;
};
const ADDorEDIT = async () => {
  if (!item.value.speakerId || !item.value.sessionId) return (error.value = true);
  loadingAdd.value = true;
  await addSpeakerToSession({ data: item.value })
    .then(async (res) => {
      router.go(-1);
    })
    .catch((error) => {
      alert(error);
    });
};
onMounted(async () => {
  loading.value = true;

  await fetchData();
  loading.value = false;
});
</script>
<template>
  <Loading v-if="loading" />
  <div class="mx-3" v-else>
    <v-card elevation="10" class="withbg">
      <v-card-item>
        <div class="d-flex align-center justify-space-between pt-sm-2">
          <v-card-title class="text-h5">Add speaker to session</v-card-title>
          <v-btn size="large" icon class="bg-primary" @click="router.go(-1)">
            <v-avatar size="large" class="text-white">
              <ArrowBackUpIcon size="25" />
            </v-avatar>
          </v-btn>
        </div>
        <v-row class="mt-3">
          <v-col cols="12" lg="12" sm="13">
            <v-select
              label="Select speaker"
              :items="speakers.data"
              :error="!item.speakerId && error"
              required
              item-title="nameEn"
              v-model="item.speakerId"
              item-value="id"
              variant="outlined"
              :return-object="false"
            ></v-select>
          </v-col>
          <v-col cols="12" lg="6" sm="13">
            <input
              type="checkbox"
              v-model="item.isModerator"
              :value="item.isModerator"
              aria-label="Hello "
            />
            Is moderator
          </v-col>
          <v-col cols="12" lg="6" sm="13">
            <input
              type="checkbox"
              v-model="item.isKeynote"
              :value="item.isKeynote"
              aria-label="Hello "
            />
            Is Keynot address
          </v-col>
        </v-row>
      </v-card-item>
      <div class="d-flex justify-end pb-3">
        <v-btn
          class="profileBtn bg-primary custom-hover-primary"
          variant="text"
          :loading="loadingAdd"
          @click="ADDorEDIT"
        >
          <span class="mr-2">Add user</span>
          <ScriptPlusIcon />
        </v-btn>
      </div>
    </v-card>
  </div>
</template>
