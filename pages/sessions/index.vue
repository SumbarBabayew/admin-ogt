...
import { getSessions, getEvents } from "@/service/admin.api";
...
const fetchItems = async () => {
  const { data } = await getSessions(JSON.parse(localStorage.getItem('SessionsQuery')) || query.value)
...
<template>
  ...
  <div v-else>
    <PageHeader :path="`/sessions/0`" :items="events" @filter="filterItem" @search="setSearch"
      @search-empty="setSearch" />
    <SessionsTable :items="items" @delete-item="setDelete" :count="count" />
