...
import { getEvents } from "@/service/admin.api";
...
const localQuery =
  JSON.parse(localStorage.getItem('EventsQuery')) ||
  {
    page: 1,
    keyword: '',
    as: "",
...
watch(route, value => {
  localStorage.setItem('EventsQuery', JSON.stringify(query.value))
...
const searchEmpty = (s) => {
  if (s) {
    query.value.keyword = ""
    query.value.page = JSON.parse(localStorage.getItem('EventsQuery')).page || 1
    localStorage.setItem('EventsQuery', JSON.stringify(query.value))
    fetchItems()
  }
}
...
const fetchItems = async () => {
  const { data } = await getEvents(JSON.parse(localStorage.getItem('EventsQuery')) || query.value)
...
<template>
  ...
  <div v-else>
    <PageHeader :path="`/events/0`" :items="product_filter" @filter="filterItem" @search="setSearch"
      @search-empty="setSearch" />
    <EventsTable :items="items" @delete-item="setDelete" :count="count" />
