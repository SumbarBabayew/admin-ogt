...
import { getHotels } from "@/service/admin.api";
import HotelCard from '@/components/dashboard/HotelCard.vue';
...
const localQuery = JSON.parse(localStorage.getItem('hotelQuery')) || { page: 1, keyword: '' }
...
watch(route, value => {
  ...
  localStorage.setItem('hotelQuery', JSON.stringify(query.value))
...
const fetchItems = async () => {
  loading.value = true
  const { data } = await getHotels(JSON.parse(localStorage.getItem('hotelQuery')) || query.value)
...
<template>
  ...
  <div v-else>
    <PageHeader :path="`/hotels/0`" :items="product_filter" @filter="filterItem" @search="setSearch" />
    <HotelCard :items="items" @delete-item="setDelete" />
