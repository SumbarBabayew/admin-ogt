...
import { getTransferRequests, getEvents } from "@/service/admin.api";
import TransferRequestTable from '@/components/tables/TransferRequestTable.vue';
...
const localQuery =
  JSON.parse(localStorage.getItem('TransferRequestsQuery')) ||
  {
    page: 1,
    keyword: '',
    as: "",
...
watch(route, value => {
  localStorage.setItem('TransferRequestsQuery', JSON.stringify(query.value))
...
const searchEmpty = (s) => {
  if (s) {
    query.value.keyword = ""
    query.value.page = JSON.parse(localStorage.getItem('TransferRequestsQuery')).page || 1
    localStorage.setItem('TransferRequestsQuery', JSON.stringify(query.value))
    fetchItems()
  }
}
...
const fetchItems = async () => {
  const { data } = await getTransferRequests(JSON.parse(localStorage.getItem('TransferRequestsQuery')) || query.value)
...
<template>
  ...
  <div v-else>
    <PageHeader :path="`/visa-request/0`" :items="events" @filter="filterItem" @search="setSearch"
      @search-empty="setSearch" />
    <TransferRequestTable :items="items" @delete-item="setDelete" :count="count" />
