...
const ADDorEDIT = async () => {
  ...
  if (route.params.id == 0) {
    await addParticipantCategory({ data: item.value, id: route.params.id })
      .then(async (res) => {
        router.go(-1);
      })
      .catch((error) => {
...
onMounted(async () => {
  ...
  if (route.params.id != 0) {
    loading.value = true;
    const { data } = await getParticipantCategory({ id: route.params.id });
    item.value = data;
    loading.value = false;
  }
});
    ...
    <v-card elevation="10" class="withbg">
      <v-card-item>
        <div class="d-flex align-center justify-space-between pt-sm-2">
          <v-card-title class="text-h5"
            >{{ route.params.id == 0 ? "Add" : "Edit" }} category</v-card-title
          >
      ...
      <div class="d-flex justify-end pb-3">
        <v-btn
          ...
          @click="ADDorEDIT"
        >
          <span class="mr-2">{{ route.params.id == 0 ? "Add" : "Edit" }} category </span>
          <ScriptPlusIcon />
        </v-btn>
      </div>
    </v-card>
