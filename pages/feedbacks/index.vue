...
import { getFeedbacks } from "@/service/admin.api";
...
const localQuery = JSON.parse(localStorage.getItem('feedbackQuery')) || { page: 1, keyword: '', categoryId: route.query.categoryId || '' }
...
watch(route, value => {
  ...
  localStorage.setItem('feedbackQuery', JSON.stringify(query.value))
...
const fetchItems = async () => {
  loading.value = true
  const { data } = await getFeedbacks(JSON.parse(localStorage.getItem('feedbackQuery')) || query.value)
...
<template>
  ...
  <div v-else>
    <PageHeader :path="`/feedbacks/0`" :items="product_filter" @filter="filterItem" @search="setSearch" />
    <FeedbackCard :items="items" @delete-item="setDelete" />
