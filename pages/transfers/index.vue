...
import { getTransfers } from "@/service/admin.api";
import TransferCard from '@/components/dashboard/TransferCard.vue';
...
const localQuery = JSON.parse(localStorage.getItem('TransfersQuery')) || { page: 1, keyword: '' }
...
watch(route, value => {
  ...
  localStorage.setItem('TransfersQuery', JSON.stringify(query.value))
...
const fetchItems = async () => {
  loading.value = true
  const { data } = await getTransfers(JSON.parse(localStorage.getItem('TransfersQuery')) || query.value)
...
<template>
  ...
  <div v-else>
    <PageHeader :path="`/transfers/0`" :items="product_filter" @filter="filterItem" @search="setSearch" />
    <TransferCard :items="items" @delete-item="setDelete" />
