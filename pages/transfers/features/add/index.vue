...
import { addTransferFeature, getTransferFeature, getTransfers } from "@/service/admin.api";
...
const fetchData = async () => {
  const speaker = await getTransfers({ limit: 100 })
...
const ADDorEDIT = async () => {
  // if (route.params.id == 0) {
    ...
    await addTransferFeature({ data: item.value, id: 0 }).then(async res => { router.go(-1) }).catch(error => { alert(error) })
  // }
  // else {
  //   if (!item.value.nameRu || !item.value.nameEn || !item.value.time) return error.value = true
  //   loadingAdd.value = true

  //   await addTransferFeature({ data: item.value, id: route.params.id }).then(async res => { router.go(-1) }).catch(error => { alert(error) })
  // }
}
onMounted(async () => {
  await fetchData();
  // if (route.params.id != 0) {
    // loading.value = true
    // const { data } = await getTransferFeature({ id: route.params.id })
    // item.value = data
    // loading.value = false
  // }
})
      ...
      <v-card-item>
        ...
        <v-row class="mt-3">
          ...
          <v-col cols="12" lg="6" sm="13">
            <v-select label="Transfers" :items="speakers.data" :error="!item.transferId && error" required
              item-title="nameEn" v-model="item.transferId" item-value="id" variant="outlined"
